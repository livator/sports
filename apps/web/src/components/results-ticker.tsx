...
import { LEAGUES, latestResults, shiftIsoDate, toLocalIsoDate, type Match } from '@sports/core';
...
/**
 * "Latest results" strip in the navigation: live and finished matches from today and
 * yesterday, a page at a time. The design shows it from 1200px up; how many results fit is
...
function TickerPages({
  ...
  const t = useTranslations('nav');
  const todays = useMatchesByDate(today);
  const yesterdays = useMatchesByDate(shiftIsoDate(today, -1));
  const results = useMemo(
    () => latestResults(todays.data ?? [], yesterdays.data ?? []),
